...
/**
 * Mapping implementation that converts struct attributes into a new instance of the specified mapped target class. The
 * mapped target class must be a top-level class and it must have a default or no-arg constructor.
 *
 * <p>
 * Attribute values are mapped based on matching the column name as obtained from result set metadata to public setters
 ...
 * short, Short, int, Integer, long, Long, float, Float, double, Double, BigDecimal, <code>java.util.Date</code>, etc.
 * 
...
public class BeanPropertyStructMapper<T> implements StructMapper<T> {
	...
	public STRUCT toStruct(T source, Connection conn, String typeName) throws SQLException {
		...
		for (int i = 0; i < values.length; i++) {
			...
			logger.info(String.format("Mapping %s to Oracle STRUCT %s", obj.getClass().getName(), typename));
